...
$(function(){
	...
	$('#hpostDiv').on('click', "img[id^='hylikeYBtn']", function(){
		if( $('#logUser_id').val() == "" ){ // 로그인 아이디 footer에 있음
			alert("로그인 후 이용해 주세요");
			return;
		};
		...
		$.ajax({
			...
			data: {
				...
				'user_id' : $('#logUser_id').val() // 로그인 아이디 footer에 있음
			},
			success: function(data){
				console.log(data.hp_like);
				$('#hylikeYBtn'+hpost_id2).hide(); // 빈하트
				$('#hylikeNBtn'+hpost_id2).show(); // 빨간하트
				$('#hylikeCnt'+hpost_id2).text(data.hp_like); // 좋아요 숫자
			}
		});
	});
	...
	$('#hpostDiv').on('click', "img[id^='hylikeNBtn']", function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#hylikeCnt'+hpost_id3).text(data.hp_like);// 좋아요 숫자
			}
		});
	});
